...
//8) ***** Telefonları console a aşağıdaki formatta yaz (4065555834)
let phone5 = [];
...
let nsOrder = extraData.filter(item => {
  return item.orderDate.includes('1996');
});

console.log(
  `1996 yılında kaç adet sipariş geçilmiştir? : ${nsOrder.length} `,
  nsOrder
);
...
let maxSale = Math.max(
  ...extraData.map(item => {
    return Math.max(
      ...item.details.map(x => x.quantity * x.unitPrice * (1 - x.discount))
    );
  })
);

console.log(`Bir seferde en yüksek satışın nedir?: ${maxSale}`);
...
let usaSale = extraData
  .filter(item => item.shipAddress.country === 'USA')
  .map(x =>
    x.details
      .map(y => y.quantity * y.unitPrice * (1 - y.discount))
      .reduce(sum, 0)
  )
  .reduce(sum, 0);
